...
inline D3DXVECTOR3 XMFloat3ToD3DXVector3(XMFLOAT3 temp){

	return D3DXVECTOR3(temp.x, temp.y, temp.z);
}

inline D3DXVECTOR2 XMFloat2ToD3DXVector2(XMFLOAT2 temp){
...
bool MeshLoader::LoadScene(const char* inFileName)
{
	//LARGE_INTEGER start;
	//LARGE_INTEGER end;
...
//D3DMATERIAL9 MeshLoader::getMaterial(){
//
//	D3DMATERIAL9 temp;
//
//	temp.Ambient.
...
void MeshLoader::ProcessGeometry(FbxNode* inNode)
{
...
void MeshLoader::ProcessSkeletonHierarchy(FbxNode* inRootNode)
{
...
void MeshLoader::ProcessSkeletonHierarchyRecursively(FbxNode* inNode, int inDepth, int myIndex, int inParentIndex)
{
...
void MeshLoader::ProcessControlPoints(FbxNode* inNode)
{
...
void MeshLoader::ProcessJointsAndAnimations(FbxNode* inNode)
{
	FbxMesh* currMesh = inNode->GetMesh();
...
unsigned int MeshLoader::FindJointIndexUsingName(const std::string& inJointName)
{
...
void MeshLoader::ProcessMesh(FbxNode* inNode)
{
	FbxMesh* currMesh = inNode->GetMesh();
...
void MeshLoader::ReadUV(FbxMesh* inMesh, int inCtrlPointIndex, int inTextureUVIndex, int inUVLayer, XMFLOAT2& outUV)
{
...
void MeshLoader::ReadNormal(FbxMesh* inMesh, int inCtrlPointIndex, int inVertexCounter, XMFLOAT3& outNormal)
{
...
void MeshLoader::ReadBinormal(FbxMesh* inMesh, int inCtrlPointIndex, int inVertexCounter, XMFLOAT3& outBinormal)
{
...
void MeshLoader::ReadTangent(FbxMesh* inMesh, int inCtrlPointIndex, int inVertexCounter, XMFLOAT3& outTangent)
{
...
int MeshLoader::FindVertex(const PNTIWVertex& inTargetVertex, const std::vector<PNTIWVertex>& uniqueVertices)
{
...
void MeshLoader::AssociateMaterialToMesh(FbxNode* inNode)
{
...
void MeshLoader::ProcessMaterials(FbxNode* inNode)
{
...
void MeshLoader::ProcessMaterialAttribute(FbxSurfaceMaterial* inMaterial, unsigned int inMaterialIndex)
{
	FbxDouble3 double3;
	FbxDouble double1;
...
void MeshLoader::ProcessMaterialTexture(FbxSurfaceMaterial* inMaterial, Material* ioMaterial)
{
...
void MeshLoader::PrintTriangles()
{
	for (unsigned int i = 0; i < mTriangles.size(); ++i)
	{
...
void MeshLoader::CleanupFbxManager()
{
	mFBXScene->Destroy();
	mFBXManager->Destroy();
...
LPD3DXMESH MeshLoader::createDXMesh(){


	LPD3DXMESH pMesh = NULL;

	D3DXCreateMesh(mTriangleCount, mVertices.size(),
	...
	for (int i = 0; i < mVertices.size(); i++){
		pVertex[i].position = XMFloat3ToD3DXVector3(mVertices[i].mPosition);
		pVertex[i].normal = XMFloat3ToD3DXVector3(mVertices[i].mNormal);
		pVertex[i].texcoord = XMFloat2ToD3DXVector2(mVertices[i].mUV);
	}
	...
	pIndex = new DWORD[mTriangles.size() * 3];
	for (int i = 0; i < mVertices.size() * 3; i+=3){
		pIndex[i] = mTriangles[i].mIndices[0];
		pIndex[i+1] = mTriangles[i].mIndices[1];
		pIndex[i+2] = mTriangles[i].mIndices[2];
	}
	...
	FBX_SAFE_DELETE_ARRAY(aAdjacency);
	return pMesh;
...
void MeshLoader::WriteMeshToStream(std::ostream& inStream)
{
...
void MeshLoader::WriteAnimationToStream(std::ostream& inStream)
{
	inStream << "<?xml version='1.0' encoding='UTF-8' ?>" << std::endl;
